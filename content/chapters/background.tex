...
When SoH drops below 70--80\%, the battery is generally considered to have reached the end of its useful automotive life. SoH can also be defined in terms of internal resistance growth: $SoH_R = (R_{EOL} - R_{current}) / (R_{EOL} - R_{new})$, where $R$ denotes internal resistance and $EOL$ stands for end of life. This thesis uses the capacity-based definition, as it directly affects driving range and matches the labeling convention of the datasets used in the experiments.
...
LightGBM offers the best balance of accuracy, computational efficiency, and deployability. The ``input flexibility'' column is particularly important: combined with feature extraction, it is the only approach in the table that simultaneously achieves high accuracy and freedom from fixed-length inputs. This directly supports one of the core contributions of this thesis --- building a single pipeline that handles variable window lengths without retraining.
...
This thesis replicates the BiLSTM baseline on the same hardware, using the thesis's own 64/16/20 cycle-level split and the first-window evaluation protocol of Sammartino et al., to ensure a fair comparison. Chapter~\ref{experiment} presents the results.
