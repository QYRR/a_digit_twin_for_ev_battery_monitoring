...
\textbf{Label units.} NBD labels are discharge capacities in ampere-hours (Ah); NRD labels are SoH percentages. In the NBD dataset, the SoH of a cell is expressed through its measured discharge capacity, following the convention of the dataset itself. This difference in scale explains the different MAE magnitudes across the two datasets (on the order of $10^{-2}$ for NBD and $10^{0}$ for NRD). The model configuration is identical; the apparent performance gap is a unit artifact.
...
\begin{table}[H]
    ...
    \hline
    Model & \makecell{MAE \\ (FW)} & \makecell{MAE \\ (Full)} & \makecell{Latency \\ \relax[ms]} & \makecell{Estimated energy \\ \relax[$\mu$J]} \\
    \hline
...
On the NBD dataset, the two methods are close at short window lengths ($wlen \leq 30$), with LightGBM holding a small edge (MAE difference below 0.21 percentage points). From $wlen = 40$ onward, BiLSTM accuracy degrades markedly: MAE jumps from 0.87\% at $wlen = 30$ to 2.16\% at $wlen = 40$, a 2.5$\times$ increase, and remains elevated at longer windows. LightGBM stays stable within a narrow band of 0.74--1.01\% across all window lengths.
