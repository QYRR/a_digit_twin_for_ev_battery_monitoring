...
\textbf{Deployment and measurement environment.} The deployment target is the SPC58EC80E5 automotive microcontroller (dual-core Power Architecture e200z420n3 at 180\,MHz, 4\,MB Flash, 384\,KB SRAM). Code is compiled with the SPC5Studio IDE's integrated GCC Power Architecture cross-compiler at optimization level \texttt{-O2}. Program execution and debugging use UDE Starterkit (PLS).

\textbf{Memory measurement.} Flash and RAM usage are derived from static analysis of the compiled object files: the LightGBM parameter arrays occupy Flash, and statically allocated program data occupy RAM. Runtime stack usage is not included, but is bounded and negligible --- tree traversal is implemented iteratively rather than recursively, so the inference call stack is only a few dozen bytes deep. The same measurement methodology is applied to the BiLSTM baseline, keeping the comparison consistent.
...
\begin{table}[H]
    ...
    SoH label range & 1.29--1.86\,Ah & 41.6--100\,\% \\
...
For the multi-window-length deployment experiment (Section~\ref{sec:exp_deployment}), a separate BiLSTM model is trained for each $wlen \in \{20, 30, 40, 50, 60\}$, because the input dimension is fixed at training time. For the generalization experiment (Section~\ref{sec:exp_generalization}), unseen window lengths are handled by trimming the oldest time steps for inputs longer than the training length, and zero-padding for shorter sequences --- since embedded hardware does not support variable-length inputs for BiLSTMs. In contrast, the LightGBM model naturally accommodates arbitrary window lengths without modification.
...
The energy figures are derived as $E = P \times t_{inf}$, assuming a nominal power consumption of 60\,mW for the SPC58 core. They are reported following the convention of Sammartino et al.~\autocite{Sammartino2021} for comparability.
...
\begin{table}[H]
    ...
    \hline
    \end{tabular}
    \caption{Deployment results. L = LightGBM (this work), B = BiLSTM, WL = Window Length. RAM\% and Flash\% are percentages of the SPC58EC80E5's 384\,KB SRAM and 4\,MB Flash.}
...
In absolute terms, both models are well within the SPC58's memory budget: the compiled LightGBM model occupies 119.5\,KB (NBD) and 126.05\,KB (NRD) of the 4\,MB Flash. Allocating 2--3\% of Flash to the model in exchange for a 10--30$\times$ inference speedup and better or equivalent accuracy is a clearly favorable trade-off for this hardware platform.
...
The experiment uses four test lengths not present in the training set: $\{10, 35, 55, 75\}$. The value 10 is shorter than the minimum training length (20); 75 is longer than the maximum (60); 35 and 55 fall within the training range but were never explicitly seen. LightGBM requires no modification to handle these lengths. BiLSTM, with its fixed input dimension, requires heuristic adaptation: inputs longer than the training length are trimmed by discarding the oldest time steps, and shorter sequences are zero-padded, as described in Section~\ref{sec:exp_setup}.

Figure~\ref{fig:generalization_heatmap} provides an overview of the generalization results for both datasets; the detailed MAE values are reported in the following subsections.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{images/ch4_generalization.png}
    \caption{MAE of the LightGBM model and the five BiLSTM variants on unseen window lengths (10, 35, 55, 75), for both datasets. Darker cells indicate lower error. The LightGBM rows remain uniformly dark across all test lengths, while the BiLSTM rows show erratic, length-dependent degradation.}
