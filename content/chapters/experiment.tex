...
The energy figures are derived as $E = P \times t_{inf}$, with the core power $P$ estimated from the device datasheet (DS11620): the typical core supply voltage $V_{DD\_LV}$ is 1.2\,V and the maximum main-core dynamic current $I_{DD\_MAIN\_CORE\_AC}$ is 50\,mA, giving $P = 1.2\,\text{V} \times 50\,\text{mA} = 60$\,mW. Because the current figure is a maximum, the energy values are a conservative upper bound. The same convention is applied to the BiLSTM baseline for comparability.
