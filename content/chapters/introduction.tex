...
This coupling points toward a \textbf{dual-model architecture}, originally proposed by Alamin~\autocite{Alamin2022}: a SoH model that tracks long-term degradation, and an SoC model that uses the SoH estimate as an input parameter. In this design, only the SoC model requires periodic over-the-air (OTA) updates — triggered when SoH has drifted by a meaningful amount — while the SoH model itself is built for long-term robustness and deployed once.
...
The Digital Twin dual-model architecture underpinning this thesis was proposed by Alamin~\autocite{Alamin2022}. Within that framework, the author's contributions are as follows:
