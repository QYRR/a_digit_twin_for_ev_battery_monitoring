...
The CPU cycle count for one complete inference (feature extraction plus model traversal) is measured. The corresponding wall-clock latency is $t_{inf} = cycles / 180\,\text{MHz}$. The BiLSTM baseline is measured under identical conditions.

\subsection{Memory Footprint}

Flash usage (model arrays, firmware code, HAL, feature extraction) and RAM usage (feature buffers, stack, HAL globals) are derived from static analysis of the compiled object files. Values are reported in absolute bytes and as percentages of the available 4\,MB Flash and 384\,KB SRAM.
